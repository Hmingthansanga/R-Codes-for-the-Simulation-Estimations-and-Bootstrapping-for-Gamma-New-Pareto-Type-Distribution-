...
neglog_likelihood <- function(log_params, x,paramb) {
...
x0<- samples
paramb <- min(x0)
xfb <- samples[samples > paramb]

# Fix the upper bound a, b and c for the parameters alpha, beta and a respectively. 1e-5 is some small positive epsilon for stability

DEresult <- DEoptim(
fn = function(params) neglog_likelihood(params, x = xfb, paramb = paramb),
lower = rep(log(1e-5), 3),
upper = c(log(a), log(b), log(c)),
control = DEoptim.control(
itermax = 1000,
trace = FALSE)
)

estimates <- c(exp(DEresult$optim$bestmem),paramb)
